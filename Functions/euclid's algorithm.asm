...
main:   push  rbp                       ; saves the caller's stack frame and creates a new one for the current function
        ...
        sub   rsp, 32                   ; makes room on stack for local variables (quadwords); a, b and c and subtracts 
                                        ; 8 additional bytes to fullfill the 16 byte stack alignment requirement
        lea   rdi, [rel scanf_format]   
        lea   rsi, [rbp-8]
        lea   rdx, [rbp-16]
        xor   rax, rax
        ...
        mov   [rbp-16], rax
        mov   [rbp-24], rax
        call  [rel scanf WRT ..got]     ; scanf returns the nr of successfully captured values in rax
        ...
        mov   rsi, rax
        xor   rax, rax
        call  [rel printf WRT ..got]    ; prints the the greatest common divisor
